class MatrixCalculator {
	...
	calculateRank() {
		...
		this.printOnConsole("Ранг матрицы: "+rank); 		
	}
	...
	invertMatrix() {		
		...
		if(this.determinantA==0) {
			this.printOnConsole("Матрица необратима.");
			return;
		}
		var adjacent = [];
		var result = [];
	...
	transposeMatrix() {
		this.rebuildMatrix();
		var string = "Результат транспонирования:\r";
		for (var i =0; i<this.AxDimension; i++) {
			for (var j=0; j<this.AyDimension; j++) {
	...
	subtractMatrix() {
		...
		if (this.AxDimension!=this.AyDimension) {
			this.printOnConsole("У матриц разная размерность.");
			return;
		}
		var result = [];
		for(var i=0; i<3; i++) 
	...
	multiplyMatrix() {
		...
		if (this.AxDimension!=this.ByDimension) {
			this.printOnConsole("Количество столбцов A отличается от количества строк B.");
			return;
		}
		var result = [];
		for(var i=0; i<3; i++) 
		...
		var string = "Результат умножения матриц:\r";
		for (i=0; i<rowsRes; i++) {
			for (j=0; j<columnsRes; j++) {
				string=string+"\t"+result[i][j];
			}
	...
	calculateDeterminant() {
		...
		if (this.AxDimension!=this.AyDimension) {
			...
			this.printOnConsole("Не квадратная матрица, определитель не может быть вычислен.");
			return;
		}
		var determinant;
		if (this.AxDimension==1) {
		...
		this.printOnConsole("Определитель: "+determinant)
		return;
	}
